...
export class FileDownloadService {
  ...
  private static async requestStoragePermission(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        '[FileDownloadService] Android 9 이하 - WRITE_EXTERNAL_STORAGE 권한 요청',
      );
      ...
      console.log(
        '[FileDownloadService] WRITE_EXTERNAL_STORAGE 권한 결과:',
        granted,
      );
  ...
  private static async getDownloadPaths(fileName: string): Promise<string[]> {
    ...
    if (Platform.OS === 'android') {
      ...
      if (RNFS.ExternalStorageDirectoryPath) {
        paths.push(
          `${RNFS.ExternalStorageDirectoryPath}/Download/${sanitizedFileName}`,
        );
      }
      ...
      if (RNFS.ExternalDirectoryPath) {
        paths.push(
          `${RNFS.ExternalDirectoryPath}/Download/${sanitizedFileName}`,
        );
      }
    ...
    try {
      ...
      if (Platform.OS === 'android' && (Platform.Version as number) <= 31) {
        try {
          ...
          const isPublicDownloads =
            publicDownloads.includes('/Download') &&
            !publicDownloads.includes('/Android/data/');
          ...
          console.log(
            '[FileDownloadService] DownloadManager 저장 완료(기본 처리):',
            savedPath2,
          );
          return { success: true, filePath: savedPath2 };
        } catch (dmError) {
          console.log(
            '[FileDownloadService] DownloadManager 실패, RNFS fallback 진행:',
            dmError,
          );
          // 계속 RNFS 경로 탐색으로 폴백
        }
      ...
      for (const downloadPath of possiblePaths) {
        try {
          ...
          // 디렉토리 생성 시도
          const dirPath = downloadPath.substring(
            0,
          ...
          console.log(
            '[FileDownloadService] 사용 가능한 경로 찾음:',
            successfulPath,
          );
          break;
        } catch (error) {
      ...
      if (!successfulPath) {
        console.error(
          '[FileDownloadService] 모든 다운로드 경로 실패:',
          lastError,
        );
        return {
          success: false,
          error:
            '파일을 저장할 수 있는 경로를 찾을 수 없습니다. 저장소 권한을 확인해주세요.',
        };
      }
      ...
      if (result.statusCode === 200) {
        console.log(
          '[FileDownloadService] 파일 다운로드 완료:',
          successfulPath,
        );
        return {
          success: true,
      ...
      if (
        ...
        return {
          success: false,
          error:
            '파일 저장 권한이 필요합니다. 설정 → 앱 권한에서 저장소 권한을 허용해주세요.',
        };
      }
